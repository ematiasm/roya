...
| An opaque server-side session vs a JWT | Opaque random token, only its SHA-256 stored, validity decided in SQL | Revocation has to be real (logout, a stolen cookie, a deactivated user) and a JWT is valid until it expires unless a denylist re-adds the very state the JWT was meant to remove; the effective permission set is resolved per request so a role edit applies immediately, which a self-contained token would make stale; and there is no signing key to provision, rotate or leak. Cost: one indexed read per request against a local SQLite file, plus a session table to prune — and no offline or cross-service verification, neither of which a single-instance application needs. Reconsider only if the app ever runs multi-instance without shared session state, or if an external service must verify identity without reaching this database; then: short TTL, refresh, denylist for revocation, and the permission set still resolved per request |
