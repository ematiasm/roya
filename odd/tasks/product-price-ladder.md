...
- **"Settings alone" was a claim about the WEB surface at U1 time, and is an application-wide claim now.** After U1 the web had one address space for a tax definition, `/web/settings/taxes…` behind `settings.manage`, and it was deliberately NOT a claim about the whole application. Since 2026-09-25 it is one: `odd/tasks/api-tax-permissions.md` narrowed the three JSON definition routes to `Require<SettingsManage>`, so the same `settings.manage` gate owns a tax definition on every surface. The bullets below are the U1 record of the narrower claim; the two JSON-side exceptions that remain are reads (`inventory.read`) and the product-tax association (`inventory.write`), and the hard delete still has no JSON route.
- **THE JSON API RESIDUAL WAS CLOSED ON 2026-09-25, OUTSIDE THIS TASK.** `odd/tasks/api-tax-permissions.md` narrowed `POST /api/taxes`, `PUT /api/taxes/{id}` and `POST /api/taxes/{id}/deactivate` to `Require<SettingsManage>`. `GET /api/taxes` and `GET /api/taxes/{id}` stayed on `inventory.read`, `POST`/`DELETE /api/products/{id}/taxes…` stayed on `inventory.write`, and no JSON hard-delete route was added. The proofs are `tax_api_definition_administration_is_exclusive_to_settings_manage` and `tax_api_preserves_inventory_read_and_association_boundaries` in `src/tax_tests.rs`.
- **OPEN DECISION, not settled by this task, closed on 2026-09-25: the JSON API residual.** `POST /api/taxes` (create), `PUT /api/taxes/{id}` (rename, re-rate, and `is_active`) and `POST /api/taxes/{id}/deactivate` in `src/routes/inventory_api.rs` are all still `Require<InventoryWrite>`. So an `inventory.write` principal — a JSON client, not a browser — can still define a tax. This is **pre-existing and is not a U1 regression**: U1 removed the second *web* catalogue, not the API, and narrowing API permissions is a separate product decision the user has not made. U1 therefore leaves those handlers and their tests untouched and records the residual instead of quietly asserting an exclusivity it does not have. Closing it means changing `/api/taxes` permissions to `settings.manage`, which is out of U1's authorized scope and needs its own decision. **This bullet is kept verbatim as the record of what was true at U1 time; the two bullets above it are what is true now.**
...
- Remove web routes and partials that become unreachable for catalogue administration, and their tests. The JSON API is unchanged — including its permissions, which this unit does not touch. **(Superseded on 2026-09-25 by `odd/tasks/api-tax-permissions.md`, which changed the JSON API's three tax-definition permissions to `settings.manage`; nothing else about the API moved.)**
- Update the authorization note so it is scoped to the web surface: after this change **no `inventory.write` web route** can create, rename, re-rate, or activate/deactivate a tax, while `settings.manage` still can. State the JSON API residual as an open decision instead of implying global exclusivity. **(The residual was stated as instructed and then closed on 2026-09-25; the authorization note in `src/routes/settings_web.rs` now states the application-wide rule instead, with the two surviving JSON exceptions named.)**
...
- [x] U1 — Remove the duplicated tax catalogue from the Products screen.
  ...
  - Prove no `inventory.write` **web** route can create, rename, re-rate, or activate/deactivate a tax, while `settings.manage` still can. The JSON API keeps its `inventory.write` tax-definition routes; that residual is recorded as an open decision, not closed here. **(That residual was closed on 2026-09-25 by `odd/tasks/api-tax-permissions.md`; the two JSON-side boundaries it deliberately left alone — read on `inventory.read`, association on `inventory.write` — are unchanged.)**
...
- [x] U3 — Run final verification and record delivery evidence.
  ...
  - Re-confirmed the web-surface tax-definition exclusivity and re-confirmed the JSON API residual as a known open decision, plus a clean money-in-JS scan over 15 authored sources. **(The JSON API residual was no longer open when this line was written on 2026-09-25: it had been closed earlier the same day in `odd/tasks/api-tax-permissions.md`. The sentence is kept as the U3-time record.)**
...
3. No `inventory.write` **web** route can create, rename, re-rate, or activate/deactivate a tax after U1. Scoped to `/web/…` on purpose: the JSON API's `POST /api/taxes`, `PUT /api/taxes/{id}` and `POST /api/taxes/{id}/deactivate` remain `Require<InventoryWrite>` and are out of this criterion's scope, left unchanged by U1 and recorded above as an open decision. **This criterion was met as written on 2026-09-25; the API half it excluded is now `settings.manage` too, so the same rule holds on every surface. Reads and product-tax association were deliberately not part of either half and did not move.**
...
- **Reachability, proved before deleting anything.** Every removed symbol had exactly one caller, all inside the catalogue:
  ...
  | `POST /web/taxes` (`web_create_tax`) | its only `hx-post` was the catalogue create form; the only `inventory.write` tax-definition create **on the web** (the JSON API's is a separate surface, untouched — **untouched by U1 only; its three definition gates became `settings.manage` on 2026-09-25**) |
  ...
  **Deliberately untouched:** `/api/taxes`, `/api/taxes/{id}` and their sub-routes (JSON API, shared with the e2e fixtures), `TaxService`, `TaxRepository`, `/web/product-taxes`, `/web/product-taxes/unlink`, `product_tax_mutation_response` (the drawer refresh), `/web/settings/taxes…`. **2026-09-25: the three JSON definition routes' permissions moved to `settings.manage`; every other item on this line is still untouched, including `TaxService`, `TaxRepository`, the association routes, and the absence of a JSON hard delete.**
...
- **Not a U1 regression.** Those API permissions are pre-existing: they predate this branch's work, and they were declared by the three `Require<InventoryWrite>` extractors in `src/routes/inventory_api.rs`. **Corrected 2026-09-25:** this line originally said they were "pinned by `tax_api_preserves_inventory_permission_boundaries`", and that was wrong. That test seeded an `inventory.read`-only principal and asserted FORBIDDEN on the three definition endpoints; no test ever asserted an `inventory.write` success on them, so it pinned the *refusal of a reader*, not the *admission of a writer*. The admission lived in the gate declarations and in this prose, and no test would have caught a change to it. U1 removed a second *web* catalogue, not the API. The defect was in the prose, not the code — which is exactly the failure mode a comment-only change can cause and a test cannot catch.
...
- **SUPERSEDED 2026-09-25 — that "nobody has made" decision was made and performed.** `odd/tasks/api-tax-permissions.md` narrowed `POST /api/taxes`, `PUT /api/taxes/{id}` and `POST /api/taxes/{id}/deactivate` to `Require<SettingsManage>`, replaced `tax_api_preserves_inventory_permission_boundaries` with `tax_api_definition_administration_is_exclusive_to_settings_manage` plus `tax_api_preserves_inventory_read_and_association_boundaries`, and rewrote all seven prose places to state the application-wide rule with the two surviving JSON exceptions. The paragraph above is kept as the U1-time record of what the correction round did then; it is not a live claim about the API today.
