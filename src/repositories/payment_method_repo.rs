...
pub trait PaymentMethodRepository: Send + Sync {
    ...
    /// account already owns 409 via UNIQUE(account_id, name).
    ...
    async fn create_in_account(&self, name: &str, account_id: i64) -> AppResult<PaymentMethod>;
...
impl PaymentMethodRepository for SqlitePaymentMethodRepository {
    async fn list_methods(&self) -> AppResult<Vec<PaymentMethod>> {
        let rows = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods ORDER BY id"#)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn find_method(&self, id: i64) -> AppResult<Option<PaymentMethod>> {
        let row = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods WHERE id = ?"#)
        .bind(id)
    ...
    async fn find_method_by_name(&self, name: &str) -> AppResult<Option<PaymentMethod>> {
        let row = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods WHERE name = ? ORDER BY id LIMIT 1"#)
        .bind(name)
        .fetch_optional(&self.pool)
        .await?;
        Ok(row.map(row_to_method))
    ...
    async fn find_method_in_account(
        ...
        let row = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods WHERE account_id = ? AND name = ?"#)
        .bind(account_id)
        .bind(name)
        .fetch_optional(&self.pool)
        .await?;
    ...
    async fn find_unassigned_by_name(&self, name: &str) -> AppResult<Option<PaymentMethod>> {
        let row = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods WHERE account_id IS NULL AND name = ? ORDER BY id LIMIT 1"#)
        .bind(name)
        .fetch_optional(&self.pool)
        .await?;
        Ok(row.map(row_to_method))
    ...
    async fn list_by_account(&self, account_id: i64) -> AppResult<Vec<PaymentMethod>> {
        let rows = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods WHERE account_id = ? ORDER BY id"#)
        .bind(account_id)
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(row_to_method).collect())
    ...
    async fn list_unassigned(&self) -> AppResult<Vec<PaymentMethod>> {
        let rows = sqlx::query(r#"SELECT id, name, account_id, is_active, created_at FROM payment_methods WHERE account_id IS NULL ORDER BY id"#)
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(row_to_method).collect())
    }
    ...
    async fn set_method_account(
        ...
        let result = sqlx::query("UPDATE payment_methods SET account_id = ? WHERE id = ?")
            .bind(account_id)
            .bind(method_id)
            .execute(&self.pool)
            .await
    ...
    async fn create_in_account(&self, name: &str, account_id: i64) -> AppResult<PaymentMethod> {
        let row = sqlx::query(r#"INSERT INTO payment_methods (name, account_id, is_active)
               VALUES (?, ?, 1)
...
mod tests {
    ...
    /// plus the `account_payment_methods` allowlist.
    ...
    async fn migration_splits_shared_methods_keeps_ids_and_drops_allowlist() {
        ...
        repo.create_in_account("Solo", b.0).await.unwrap();
        let dup = repo.create_in_account("Solo", b.0).await.unwrap_err();
    ...
    async fn set_method_account_assigns_unassigns_and_rejects_unknowns() {
        ...
        let repo = SqlitePaymentMethodRepository::new(pool.clone());
        let acc: (i64,) = sqlx::query_as("INSERT INTO accounts (name) VALUES ('A') RETURNING id")
            .fetch_one(&pool)
            .await
            .unwrap();
        ...
        repo.set_method_account(cash.id, Some(acc.0)).await.unwrap();
        assert_eq!(repo.find_method(cash.id).await.unwrap().unwrap().account_id, Some(acc.0));
        repo.set_method_account(cash.id, None).await.unwrap();
        assert_eq!(repo.find_method(cash.id).await.unwrap().unwrap().account_id, None);

        let err = repo.set_method_account(999_999, Some(acc.0)).await.unwrap_err();
        assert!(matches!(err, AppError::NotFound(_)), "got {err:?}");
        let err = repo.set_method_account(cash.id, Some(999_999)).await.unwrap_err();
        assert!(matches!(err, AppError::NotFound(_)), "got {err:?}");
    }
}
