    ...
    PM: crate::repositories::PaymentMethodRepository,
{
    ...
    /// The documents drawer's draft delete — the mirror of the sale flow. A
    /// draft is the ONE deletable state: it never touched stock, money or a
    /// supplier's debt — payments only exist on a Confirmed document, and the
    /// stock movement and the ledger entry are both created by `confirm` — so
    ...
    pub async fn delete_draft(&self, id: i64) -> AppResult<()> {
        ...
        if purchase.status != PurchaseStatus::Draft {
            return Err(AppError::Validation(format!(
                "purchase {id} is {}: only a draft can be deleted",
                purchase.status
            )));
        }
        ...
        if !deleted {
            ...
            return Err(AppError::Validation(format!(
                "purchase {id} is no longer a draft: only a draft can be deleted"
            )));
        }
        Ok(())
    }
...
mod tests {
    ...
    async fn ac4_credit_payments_post_expenses_overpay_rejected_paid_at_zero_due() {
        ...
        s.record_payment(audit_actor(&s).await, purchase.id, cash, dec("15"), d(2024, 5, 10))
            .await
            .unwrap();
        ...
        let err = s
            .record_payment(audit_actor(&s).await, purchase.id, cash, dec("30"), d(2024, 5, 11))
            .await
            .unwrap_err();
        ...
        s.record_payment(audit_actor(&s).await, purchase.id, cash, dec("25"), d(2024, 5, 12))
            .await
            .unwrap();
    ...
    async fn t5_pay_supplier_covers_oldest_first_with_exact_dues() {
        ...
        let payments = s
            .pay_supplier(audit_actor(&s).await, sup.id, cash, dec("55"), d(2024, 6, 20))
            .await
            .unwrap();
        ...
        let rest = s
            .pay_supplier(audit_actor(&s).await, sup.id, cash, dec("15"), d(2024, 6, 21))
            .await
            .unwrap();
    ...
    async fn t5_pay_supplier_overpay_rejected_with_no_side_effect() {
        ...
        let err = s
            .pay_supplier(audit_actor(&s).await, sup.id, cash, dec("71"), d(2024, 6, 20))
            .await
            .unwrap_err();
        match &err {
    ...
    async fn t5_pay_supplier_unknown_supplier_is_404() {
        let (s, _) = svc().await;
        let err = s
            .pay_supplier(audit_actor(&s).await, 999_999, 1, dec("10"), purchase_date())
            .await
            .unwrap_err();
    ...
    async fn t5_pay_supplier_unassigned_method_is_400() {
        ...
        let err = s
            .pay_supplier(audit_actor(&s).await, sup.id, cash, dec("5"), purchase_date())
            .await
            .unwrap_err();
        assert!(matches!(err, AppError::Validation(_)), "got {err:?}");
        assert_eq!(tx_count(&pool).await, 0, "no finance may be posted");
    ...
    async fn ac7_cancel_confirmed_returns_stock_and_refunds_without_balance_guard() {
        ...
        let cancelled = s
            .cancel(audit_actor(&s).await, purchase.id, Some(" devuelvo ".into()))
            .await
            .unwrap();
    ...
    async fn ac11_stock_and_finance_rows_reference_purchase_number() {
        ...
        let cash = cash_method(&s).await;
        allow(&s, acc.id, cash).await;
        s.record_payment(audit_actor(&s).await, purchase.id, cash, dec("5"), d(2024, 5, 20))
            .await
            .unwrap();
        let rows = s
            .transactions
    ...
    async fn ac14_unassigned_method_rejected_without_side_effects() {
        ...
        let err = s
            .record_payment(audit_actor(&s).await, credit.id, qr, dec("5"), purchase_date())
            .await
            .unwrap_err();
        assert!(matches!(err, AppError::Validation(_)), "got {err:?}");
    ...
    async fn tri_draft_cancel_is_noop_and_keeps_number_null() {
        ...
        let cancelled = s.cancel(audit_actor(&s).await, purchase.id, Some("ya no".into())).await.unwrap();
        assert_eq!(cancelled.purchase.status, PurchaseStatus::Cancelled);
        ...
        let err = s.cancel(audit_actor(&s).await, purchase.id, None).await.unwrap_err();
        assert!(matches!(err, AppError::Validation(_)), "got {err:?}");
    }
    ...
    async fn tri_cancel_confirmed_refunds_each_originating_account() {
        ...
        s.record_payment(audit_actor(&s).await, purchase.id, cash, dec("15"), d(2024, 5, 10))
            .await
            .unwrap();
        s.record_payment(audit_actor(&s).await, purchase.id, transfer, dec("25"), d(2024, 5, 11))
            .await
            .unwrap();

        s.cancel(audit_actor(&s).await, purchase.id, None).await.unwrap();
        let rows_a = s
            .transactions
            .transactions
    ...
    async fn tri_confirm_rejects_duplicate_product_lines_without_side_effects() {
        ...
        let err = s.confirm(audit_actor(&s).await, purchase.id, None).await.unwrap_err();
        assert!(matches!(err, AppError::Validation(_)), "got {err:?}");
    ...
    async fn link_credit_payment_and_cancel_refund_keeps_original_transaction() {
        ...
        let paid = s
            .record_payment(audit_actor(&s).await, purchase.id, cash, dec("15"), d(2024, 5, 10))
            .await
            .unwrap();
        let paid_tx_id = paid
            .transaction_id
        ...
        s.cancel(audit_actor(&s).await, purchase.id, Some("return".into())).await.unwrap();
